...
export const Auth = ({ type }) => {
  ...
  async function sendRequest() {
    ...
    try {
      ...
      const response = await axios.post(
        `${REACT_APP_BACKEND_URL}/api/v1/user/${type}`,
        {
          username: username,
          password: password,
        }
  ...
  async function handleDemoLogin() {
    const demo_user = "alpha"; //need to env variable later
    const demo_password = "alpha1234"; //need to env variable later
    try {
